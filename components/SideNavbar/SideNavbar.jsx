...
`},
  ...
  return (
    ...
    <div className={`${isSideBarOpen ? "w-72": "w-20"} sticky top-0 border-2 border-slate pt-8 p-5 h-screen bg-veryDarkGray duration-500`}>
            <div className={`absolute cursor-pointer text-white bg-veryDarkGray -right-3 top-9 w-7 border-2  rounded-full border-slate ${!isSideBarOpen && 'rotate-180'}`} onClick={() => setIsSideBarOpen(!isSideBarOpen)}>
                ...
                {Menus.map((menu, index) =>(
                   <Link href={menu.link} key={index} className='flex cursor-pointer p-2 pl-0 hover:bg-slate'>
                     <li key={index} className='text-sm flex items-center gap-x-4 pl-1'>
                        ...
                        <span className={`${!isSideBarOpen ? "hidden" : ""} origin-left text-white  duration-300`}>{menu.title}</span>
                    </li>
                   </Link>
                ) )}
            </ul>
